...
{%- if columns is string -%}
    ...
    {%- else -%}
        {%- set escaped_column_str = dbtvault.escape_column_names(column_str) -%}
    {%- endif -%}
    {{- "CAST(({}({})) AS STRING) AS {}".format(hash_alg, standardise | replace('[EXPRESSION]', escaped_column_str), dbtvault.escape_column_names(alias)) | indent(4) -}}
...
{%- else -%}
    ...
    {%- for column in columns -%}
        ...
        {%- if loop.last -%}
            ...
            {% if is_hashdiff %}
                {{- "\n)) AS STRING) AS {}".format(dbtvault.escape_column_names(alias)) -}}
            {%- else -%}
                {{- "\n), '{}')) AS STRING) AS {}".format(all_null | join(""), dbtvault.escape_column_names(alias)) -}}
            {%- endif -%}
        {%- else -%}
